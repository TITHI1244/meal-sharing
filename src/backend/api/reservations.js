...
router.post("/", async (request, response) => {
  try {
    ...
    .insert({ number_of_guests: request.body.number_of_guests,
...
router.get("/:id", async (request, response) => {
  try {
    ...
    const requestedReservation = await knex("reservation")
                    .select('number_of_guests', 'meal_id', 'created_date', 'contact_phonenumber', 'contact_name', 'contact_email') 
...
router.put("/:id", async (request, response) => {
  try {
    ...
    await knex("reservation")
                    .where({id : request.params.id})
                    .update({meal_id: 2})
                    .then(item => response.json(reservations.filter(reservation => reservation.id === parseInt(request.params.id))));
  } catch (error) {
    throw error;
  }
});
